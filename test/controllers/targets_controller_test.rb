...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { description: @target.description, title: @target.title, user_id: @target.user_id } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { description: @target.description, title: @target.title, user_id: @target.user_id } }
